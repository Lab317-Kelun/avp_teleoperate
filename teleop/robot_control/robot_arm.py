...
# 导入 Unitree DDS 通信接口相关模块
from unitree_sdk2py.core.channel import ChannelPublisher, ChannelSubscriber, ChannelFactoryInitialize
from unitree_sdk2py.idl.unitree_hg.msg.dds_ import ( LowCmd_  as hg_LowCmd, LowState_ as hg_LowState)
from unitree_sdk2py.idl.default import unitree_hg_msg_dds__LowCmd_
from unitree_sdk2py.utils.crc import CRC

from unitree_sdk2py.idl.unitree_go.msg.dds_ import ( LowCmd_  as go_LowCmd, LowState_ as go_LowState)
from unitree_sdk2py.idl.default import unitree_go_msg_dds__LowCmd_

sys.path.append('/home/asus/unitreerobotics/avp_teleoperate/teleop/robot_control')
# lcm 通信包
from arm_msg import arm_action_lcmt

# STEP 1
kTopicLowCommand = "rt/lowcmd"  # 控制命令的 DDS topic
#kTopicLowCommand = "rt/arm_sdk"
kTopicLowState = "rt/lowstate"  # 电机状态反馈的 DDS topic
...
class G1_29_ArmController:
    def __init__(self):
        ...
        # 初始化控制目标
        self.q_target = np.zeros(14)          # 目标角度
        ...
        self.arm_velocity_limit = 20.0        # 初始速度限制
    ...
    def _ctrl_motor_state(self):
        '''主控制循环，发送目标角度和力矩到 DDS'''
        
        # 初始化LCM通信
        lc = lcm.LCM("udpm://239.255.76.67:7667?ttl=255")
        # 定义上半身发送的数据
        msg_send = arm_action_lcmt()
        ...
        while True:
            ...
            #     self.msg.motor_cmd[id].q   = cliped_arm_q_target[idx]  # 目标角度（position）
            #     self.msg.motor_cmd[id].dq  = 0                         # 目标角速度（velocity）
            #     self.msg.motor_cmd[id].tau = arm_tauff_target[idx]     # 力矩控制指令（torque）
            
            
            # 发送数据赋值
            # 左臂各关节角度
            msg_send.act[0] = cliped_arm_q_target[0]
            msg_send.act[1] = cliped_arm_q_target[1]
            msg_send.act[2] = cliped_arm_q_target[2]
            msg_send.act[3] = cliped_arm_q_target[3]
            msg_send.act[4] = cliped_arm_q_target[4]
            msg_send.act[5] = cliped_arm_q_target[5]
            msg_send.act[6] = cliped_arm_q_target[6]
            
            # 左臂各关节力矩
            msg_send.torques[0] = arm_tauff_target[0] 
            msg_send.torques[1] = arm_tauff_target[1] 
            msg_send.torques[2] = arm_tauff_target[2] 
            msg_send.torques[3] = arm_tauff_target[3] 
            msg_send.torques[4] = arm_tauff_target[4] 
            msg_send.torques[5] = arm_tauff_target[5] 
            msg_send.torques[6] = arm_tauff_target[6] 
        
            # 右臂各关节角度
            msg_send.act[7] = cliped_arm_q_target[7]
            msg_send.act[8] = cliped_arm_q_target[8]
            msg_send.act[9] = cliped_arm_q_target[9]
            msg_send.act[10] = cliped_arm_q_target[10]
            msg_send.act[11] = cliped_arm_q_target[11]
            msg_send.act[12] = cliped_arm_q_target[12]
            msg_send.act[13] = cliped_arm_q_target[13]
    
            # 右臂各关节力矩
            msg_send.torques[7] = arm_tauff_target[7] 
            msg_send.torques[8] = arm_tauff_target[8] 
            msg_send.torques[9] = arm_tauff_target[9] 
            msg_send.torques[10] = arm_tauff_target[10] 
            msg_send.torques[11] = arm_tauff_target[11] 
            msg_send.torques[12] = arm_tauff_target[12] 
            msg_send.torques[13] = arm_tauff_target[13] 

            # print('msg_send',msg_send.act[0])
            lc.publish("arm_action", msg_send.encode())
            # print('meg_act:', msg_send.act)
            # print('meg_torques:', msg_send.torques)

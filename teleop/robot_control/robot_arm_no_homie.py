...
class G1_29_ArmController:
    ...
    def ctrl_right_arm(self, q_target_right, tauff_target_right):
        '''只设置右臂的目标角度和目标力矩，左臂保持当前位置'''
        with self.ctrl_lock:
            self.q_target = np.concatenate([np.zeros(7), q_target_right])
            self.tauff_target = np.concatenate([np.zeros(7), tauff_target_right])
